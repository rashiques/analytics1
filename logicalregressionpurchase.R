...
logitmodel1 = glm(purchased ~ gender + age + salary, family = binomial,  data = training_set)#glm   for logical regerssion were purchase is dependent
summary(logitmodel1)
...
length(prob_pred)
summary(prob_pred)
head(cbind(test_set,prob_pred ),10)# there is inaccuracy here
...
cm = table(test_set[,5], y_pred)#it print 5th colt that is purchase and predicted
cm
(accuracy = (60 + 24)/ sum(cm))

library(caret)
library(e1071)
caret::confusionMatrix(cm)#it automaticaly give confusion matrix
