...
head(titanic)
names(titanic)#sibsp is number of siblings
View(titanic)
...
fit
rpart.plot(fit, extra = 106, cex=.8,nn=T)  #plot nn is node number

printcp(fit) #select complexity parameter
prunetree2 = prune(fit, cp=.014)
rpart.plot(prunetree2, cex=.8,nn=T)#CEX IS 0.8 is amplification
prunetree2
nrow(data)
