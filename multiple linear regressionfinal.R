...
omni2a=read.csv('./data/salesquantity.csv')
head(omni2a)
#you don't know the location of file
omni2b=read.csv(file.choose())#searching the file
head(omni2b)
...
fit2 = lm(sales ~ price + promotion, data=omni)#sales with respect to peice and promotion
...
plot(fit2,4)  # tells outliers which affect model(spoil the model)
omni[-c(11,14,15),]

fit3 = lm(sales ~ price + promotion, data=omni[-c(11,14,15),])
plot(fit3,4)
summary(fit3)# rsquare is improved=0.8257
summary(fit2)# rsquare=0.7421
